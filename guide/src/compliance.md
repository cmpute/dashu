...
| Each component rounded independently to the shared mode | ✅ | Correctly rounded per axis via a Ziv loop that certifies both parts, mirroring `dashu-float`'s real transcendentals (`asin`/`acos`/`atan` lose accuracy only very near their singularities, `z = ±1`/`±i`). |
