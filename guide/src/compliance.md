...
| Correct rounding to within 1 ulp | ✅ | All operations guarantee $|error| < 1\text{ ulp}$. Arithmetic, roots, and the real transcendentals (`exp`, `exp_m1`, `ln`, `ln_1p`, the trigonometric and hyperbolic families including the ×u/×π variants, `hypot`, `powi`, and `powf`) are guaranteed-correctly rounded via a Ziv loop; and `dashu-cmplx`'s complex transcendentals (`exp`, `log`, `powf`, `powi`, `sin`/`cos`/`tan`/`sin_cos` including the ×π variants, `asin`/`acos`/`atan`, the hyperbolic `sinh`/`cosh`/`tanh`/`sinh_cosh` and `asinh`/`acosh`/`atanh`, `sqrt`) are correctly rounded via their own Ziv loop that certifies both parts (`asin`/`acos` use the factored $1-z^2=(1-z)(1+z)$, which keeps the cancellation out of the formula itself; the hyperbolic family reuses the circular one through the exact rotation identities `sinh z = -i\sin(iz)` etc., inheriting its certification). The `Rounded` type distinguishes exact from inexact results. |
...
| Each component rounded independently to the shared mode | ✅ | Correctly rounded per axis via a Ziv loop that certifies both parts, mirroring `dashu-float`'s real transcendentals (`asin`/`acos` use the factored $1-z^2=(1-z)(1+z)$ so they stay accurate right up to the singularities $z = \pm 1$). |
