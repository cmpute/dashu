...
- **Correctly-rounded transcendentals.** The real transcendentals — `exp`, `exp_m1`, `ln`, `ln_1p`, the trigonometric and hyperbolic families, `hypot`, and `powf` (non-integer exponent) — are guaranteed-correctly rounded via a Ziv retry loop. An integer-valued `powf` exponent delegates to `powi` (binary exponentiation, within 1 ulp). `dashu-cmplx`'s complex transcendentals (`exp`, `log`, `powf`, `sin`/`cos`/`tan`/`sin_cos`, `asin`/`acos`/`atan`, `sqrt`) are correctly rounded via their own Ziv loop that certifies both parts; `tan`/`asin`/`acos`/`atan` lose accuracy only very near their poles/singularities.
