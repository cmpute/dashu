...
impl_builtin_distr!(impl Standard for RBig, new, sample_rbig);
impl_builtin_distr!(impl Standard for Relaxed, new, sample_relaxed);
...
impl UniformSampler for UniformRBig {
    ...
    #[inline]
    fn new<B1, B2>(low: B1, high: B2) -> UniformRBig
    where
        B1: SampleBorrow<RBig> + Sized,
        B2: SampleBorrow<RBig> + Sized,
    {
        let (low_n, high_n, den) = UniformRBig::parse_bounds(low.borrow(), high.borrow());
        UniformRBig::from_parts(UniformIBig::new(low_n, high_n), den)
    }

    #[inline]
    fn new_inclusive<B1, B2>(low: B1, high: B2) -> UniformRBig
    where
        B1: SampleBorrow<RBig> + Sized,
        B2: SampleBorrow<RBig> + Sized,
    {
        let (low_n, high_n, den) = UniformRBig::parse_bounds(low.borrow(), high.borrow());
        UniformRBig::from_parts(UniformIBig::new_inclusive(low_n, high_n), den)
    }
