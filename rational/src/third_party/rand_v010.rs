//! Random rational numbers generation with the `rand` crate (0.10.x).
//!
...
//! of the builtin `rand` distributions [StandardUniform], [Open01], [OpenClosed01]. The other one is
...
//!
//! ```
//! use dashu_ratio::{RBig, Relaxed, rand_v010::Uniform01};
//! # use rand_v010::{distr::uniform::Uniform, rng, Rng, RngExt};
//!
//! // generate RBigs in a [0, 1) or [0, 1] with a given precision
//! let a: RBig = rng().sample(Uniform01::new(&10u8.into()));
//! let b: Relaxed = rng().sample(Uniform01::new_closed(&10u8.into()));
//! let c: RBig = rng().random(); // the default distribution generates in [0, 1)
//! assert!(a >= RBig::ZERO && a < RBig::ONE);
//! assert!(b >= Relaxed::ZERO && b <= Relaxed::ONE);
//! assert!(c >= RBig::ZERO && c < RBig::ONE);
//!
//! // generate RBigs in a range
//! let a = rng().random_range(RBig::from(3)..RBig::from(10));
//! let b = rng().sample(Uniform::new(RBig::from(-5), &a).unwrap());
...
//! * [StandardUniform], [Open01], [OpenClosed01] will generate rationals with [DoubleWord::MAX] as
//!   the denominator.
...
use dashu_base::Gcd;
use dashu_int::{
    rand_v010::{UniformBelow, UniformIBig},
...
/// A uniform distribution between 0 and 1. It can be used to replace the [StandardUniform], [Open01],
...
impl_builtin_distr!(impl StandardUniform for RBig, new);
impl_builtin_distr!(impl StandardUniform for Relaxed, new);
