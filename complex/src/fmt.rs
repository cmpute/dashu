...
mod tests {
    ...
    fn display_signed_zero() {
        ...
        // the surviving component inherits FBig's `-0` rule — `-0` always carries its sign, as
        // `f64`'s does.
        ...
        // a vanishing ±0 term is dropped; the surviving -0 keeps its sign
        assert_eq!(format!("{}", mk(n0(), p0())), "-0"); // (-0, 0)
        assert_eq!(format!("{}", mk(p0(), n0())), "0"); // ( 0,-0) im dropped, re is +0
        ...
        // the `+` flag is a no-op on an already-signed zero
