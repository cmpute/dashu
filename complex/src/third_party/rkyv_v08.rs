...
//! (`cfg_attr` on `Context`/`CBig`); this module only holds the round-trip tests. Gated on
...
mod tests {
    ...
    fn cbig_roundtrip() {
        ...
        for z in &values {
            let bytes = to_bytes::<rkyv_v08::rancor::Error>(z).unwrap();

            // SAFETY: `bytes` came from `rkyv::to_bytes` on the same value, so it is a valid

            // archived object with the root at the end.

            let z2 = unsafe { from_bytes_unchecked::<C, rkyv_v08::rancor::Error>(&bytes) }.unwrap();
            assert_eq!(z2, *z, "CBig round-trip failed for {z}");
            assert_eq!(z2.precision(), z.precision());
        }
    }
}
