...
fn test_cmp_across_representations() {
    ...
    let ubig_cases = [
        ...
        // ones() must canonicalize 128-bit values to the inline form, so the
        // scale short-circuit never misorders equal values
        (UBig::ones(128), UBig::from(u128::MAX), Equal),
        (UBig::ones(256), (ubig!(1) << 256) - ubig!(1), Equal),
    ];
