...
mod tests {
    ...
    // Micro-benchmark of portable vs BMI2 addmul-2 kernel throughput.
    // Interleaved rounds, reporting the per-method minimum to dampen thermal /
    // ordering noise. Run with: `cargo test --release bench_dword_kernel -- --ignored --nocapture`.
    #[cfg(all(target_arch = "x86_64", feature = "std"))]
    #[test]
    #[ignore]
    fn bench_dword_kernel_bmi2_vs_portable() {
        use std::time::Instant;
        let n = 512usize;
        let rhs: Vec<u64> = (0..n as u64)
            .map(|i| i.wrapping_mul(0x9E3779B97F4A7C15))
            .collect();
        let m0 = 0x243F_6A88_85A3_08D3;
        let m1 = 0x9738_1146_A3C0_B101;
        let iters = 1_000_000;
        let rounds = 7;

        if !std::is_x86_feature_detected!("bmi2") {
            eprintln!("bmi2 not available on this CPU; skipping");
            return;
        }

        // The kernel has no value-dependent control flow, so calling it
        // repeatedly on the same buffer measures pure kernel throughput.
        let run = |portable: bool| -> u128 {
            let mut w = rhs.clone();
            let start = Instant::now();
            let mut acc = 0u64;
            for _ in 0..iters {
                let (cl, ch) = if portable {
                    add_mul_dword_same_len_in_place_impl(&mut w, &rhs, m0, m1)
                } else {
                    // SAFETY: bmi2 support was confirmed before this closure is
                    // ever called with `portable == false`.
                    unsafe { add_mul_dword_same_len_in_place_bmi2(&mut w, &rhs, m0, m1) }
                };
                acc ^= cl ^ ch;
            }
        ...
        let mut min_b = u128::MAX;
        for r in 0..rounds {
            // Alternate which kernel runs first each round, to cancel out
            // boost decay, but always keep `tp` = portable time, `tb` = bmi2 time.
            let (tp, tb) = if r % 2 == 0 {
                (run(true), run(false))
            } else {
                let tb = run(false);
            ...
            min_p = min_p.min(tp);
            min_b = min_b.min(tb);
            eprintln!("round {r}: portable={tp}ns bmi2={tb}ns");
        }
        eprintln!(
            "n={n} iters={iters} BEST portable={min_p}ns bmi2={min_b}ns ratio(portable/bmi2)={:.3}x",
            min_p as f64 / min_b as f64,
        );
    }
}
