...
- ×u 变体：`sin_unit(u)`、`cos_unit(u)`、`tan_unit(u)` 以及 `sin_cos_unit(u)`——计算 `2π·x/u`，即以整圈的 u 分之一为单位度量角度，反函数为 `asin_unit(u)`、`acos_unit(u)`、`atan_unit(u)` 与四象限的 `atan2_unit`（以 `y.atan2_unit(x, u)` 形式调用，接收者是 y 坐标，第一个参数是 x 坐标）。`u = 2` 即 ×π 函数（`sin_pi` 等）；`u = 360` 即角度（度数）。
...
×u 圆函数在整数运算中对参数做*精确的* mod u 约化，因此与弧度函数不同，其精度不会随输入增大而下降（`sin_unit(10^100, 4)` 精确等于 `0`：`10^100` 是 `4` 的倍数）。满足 `12x/u`（正切为 `8x/u`）为整数的参数得到精确结果：四分之一与八分之一点映射到 `0`/`±1`（`sin_unit(90, 360) == 1`），六分之一点映射到 `±1/2`（`sin_unit(30, 360) == 0.5`）。在 `u/4` 的奇数倍处正切到达极点，两侧单侧极限分别为 `+∞` 和 `−∞`：该情形是不定的，以错误形式报告。反函数族在坐标轴与对角线上返回精确的 `k·u/8` 值（`(1).atan2_unit(&(1), 360) == 45`），且 `u → 0` 时每个（域内）反函数的极限是有符号零。
