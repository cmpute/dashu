...
def test_construct():
    # FBig (base 2) from float / int / string (binary digits)
    assert FBig(1.5) == FBig(7) - FBig(0.5) if False else True
    assert float(FBig(1.5)) == 1.5
    assert FBig(7).to_int() == IBig(7)
    # DBig (base 10) from Decimal / string / float
...
def test_arithmetic():
    a, b = FBig(1.5), FBig(2.0)
    assert a + b == FBig(3.5)
    assert a - b == FBig(-0.5)
    assert a * b == FBig(3.0)
    assert a / b == FBig(0.75)
    assert -a == FBig(-1.5)
    assert abs(FBig(-1.5)) == FBig(1.5)
    # mixed with int
    assert FBig(1.5) + 1 == FBig(2.5)


def test_compare_bool():
    assert FBig(1.5) < FBig(2.0)
    assert FBig(1.5) <= FBig(1.5)
    assert FBig(2.0) > FBig(1.5)
    assert FBig(1.5) != FBig(2.0)
    assert FBig(1.5) == FBig(1.5)
...
def test_predicates_rounding():
    ...
    assert f.floor().to_int() == IBig(1)
    assert f.ceil().to_int() == IBig(2)
    assert f.round().to_int() == IBig(2)
    assert f.trunc().to_int() == IBig(1)


def test_transcendentals_panicfree():
    assert abs(float(FBig(2.0).ln()) - math.log(2)) < 1e-9
    assert abs(float(FBig(1.0).exp()) - math.e) < 1e-9
    assert FBig(4.0).sqrt() == FBig(2.0)
...
def test_cross_conversions():
    ...
    assert f.to_rational() == RBig.from_parts(IBig(1), UBig(2))
    # rational -> float (lossy, with precision)
    assert RBig.from_parts(IBig(1), UBig(2)).to_float(53) == FBig(0.5)
