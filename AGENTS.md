...
- **`dashu-float` and `dashu-cmplx` are expected to be correctly rounded.** The enforcement lives in the `fuzz/` differentials, and its strength is per suite: the float directed tests (`fuzz/tests/float_transcendental.rs`) assert **bit-exact** agreement with MPFR under every rounding mode; the complex tests (`fuzz/tests/cmplx_transcendental.rs`) assert bit-exact per-component agreement with MPC for the families covered by directed checks, and keep a documented ulp tolerance (`CLOSE_K`) where a bit-exact oracle is unavailable (the ×π family has no MPC entry; MPC's `pow` is not guaranteed correctly rounded). Every nearest-mode test that is not directed shares the same tolerance — `fuzz::CLOSE_K` = 2 ulps for float transcendentals and per component for complex — so the two suites stay equally strict. When a fuzz run finds a discrepancy, **fix the implementation, not the fuzz test** — relaxing a tolerance to hide a failure is a regression.
