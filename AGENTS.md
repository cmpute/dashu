...
- **`dashu-float` and `dashu-cmplx` are expected to be correctly rounded**, and the fuzz differentials (under `fuzz/`) enforce this strictly — the float tests assert bit-exact agreement with MPFR under every rounding mode (not a loose ulp tolerance). The complex differentials currently assert agreement with MPC to within 16 ulps per component (nearest modes); tightening them to bit-exact is planned (`float/PLAN_PHASE2.md`). When a fuzz run finds a discrepancy, **fix the implementation, not the fuzz test** — relaxing a tolerance to hide a failure is a regression.
