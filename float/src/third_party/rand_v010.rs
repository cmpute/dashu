//! Random floating point number generation with the `rand` crate (0.10.x).
//!
...
//!
//! ```
//! use dashu_float::rand_v010::Uniform01;
...
//! let a: FBig = rng().sample(Uniform01::new(10));
//! let b: FBig = rng().sample(Uniform01::new_closed(10));
//! let c: FBig = rng().random(); // the default distribution generates in [0, 1)
//! assert!(a >= FBig::ZERO && a < FBig::ONE);
//! assert!(b >= FBig::ZERO && b <= FBig::ONE);
//! assert!(c >= FBig::ZERO && c < FBig::ONE);
//!
//! // generate FBigs in a range
//! let a = rng().random_range(FBig::from(3)..FBig::from(10));
//! let b = rng().sample(Uniform::new(FBig::from(-5), &a).unwrap());
...
//! * [StandardUniform], [Open01], [OpenClosed01] will generate floats with the max precision
//!   such that the significand fits in a [DoubleWord].
//! * [UniformFBig] (and therefore [Uniform][rand_v010::distr::Uniform]) will generate floats
...
use dashu_int::{
    rand_v010::{UniformBelow, UniformBits},
    DoubleWord, UBig,
};
use rand_v010::{
    distr::{
