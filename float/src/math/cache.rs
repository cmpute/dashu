...
impl<const B: Word> Default for MathCache<B> {
    #[inline]
    fn default() -> Self {
        Self::new()
    }
...
impl<const B: Word> MathCache<B> {
    ...
    /// Number of significant base-`B` digits required to represent `bits` bits.
    /// Mirrors the bit-counting logic in [`Context::pi`](crate::repr::Context::pi).
    fn bits_for_precision(precision: usize) -> usize {
        if B.is_power_of_two() {
            precision.saturating_mul(B.ilog2() as usize)
        } else {
            precision.saturating_mul(B.ilog2() as usize + 1)
        }
    }

    /// Convert a work-precision expressed in bits back to base-`B` digits.
    fn precision_for_bits(bits: usize) -> usize {
        if B == 2 {
            bits
        } else {
            bits / B.ilog2() as usize + 1
        }
    }
    ...
    /// let _pi_100 = cache.pi::<HalfAway>(100).value();
    /// // second call at higher precision extends the cached state
    /// let pi_1000 = cache.pi::<HalfAway>(1000).value();
    ...
    pub fn pi<R: Round>(&self, precision: usize) -> Rounded<FBig<R, B>> {
        ...
        let bits = Self::bits_for_precision(precision);
        ...
        let work_precision = Self::precision_for_bits(bits + guard_bits);
    ...
    pub fn ln2<R: Round>(&self, precision: usize) -> FBig<R, B> {
        ...
        let work = precision + Self::combine_guard(precision);
        let l6 = self.iacoth::<R>(6, work);
        let l99 = self.iacoth::<R>(99, work);
    ...
    ///
    /// let cache: MathCache<10> = MathCache::new();
    /// let ln10 = cache.ln10::<HalfAway>(50);
    ...
    pub fn ln10<R: Round>(&self, precision: usize) -> FBig<R, B> {
        ...
        let work = precision + Self::combine_guard(precision);
        let l2 = self.ln2::<R>(work);
        let l9 = self.iacoth::<R>(9, work);
    ...
    /// let cache: MathCache<2> = MathCache::new();
    /// let ln_base = cache.ln_base::<HalfAway>(50);
    /// let ln2 = cache.ln2::<HalfAway>(50);
    ...
    pub fn ln_base<R: Round>(&self, precision: usize) -> FBig<R, B> {
        match B {
            ...
            b if b.is_power_of_two() => {
                ...
                (FBig::from(bits) * self.ln2::<R>(work))
                    .with_precision(precision)
                    .value()
            }
            _ => {
...
mod tests {
    ...
    fn test_pi_matches_context() {
        ...
        for &precision in &[10usize, 50, 100] {
            let cache: MathCache<10> = MathCache::new();
            let cached = cache.pi::<mode::HalfEven>(precision).value();
    ...
    fn test_pi_lower_precision_reuses() {
        ...
        let cache: MathCache<10> = MathCache::new();
        let _pi_high = cache.pi::<mode::HalfEven>(200).value();
        // the slot now holds >=200 terms; a 50-digit request reuses it
        let pi_50 = cache.pi::<mode::HalfEven>(50).value();
    ...
    fn test_pi_extension_matches_scratch() {
        ...
        let cache: MathCache<10> = MathCache::new();
        let _pi_100 = cache.pi::<mode::HalfAway>(100).value();
        let pi_1000_extended = cache.pi::<mode::HalfAway>(1000).value();
    ...
    fn test_iacoth_extension_matches_scratch() {
        ...
        let cache: MathCache<10> = MathCache::new();
        let _ln2_low = cache.ln2::<mode::HalfAway>(20);
        let ln2_high = cache.ln2::<mode::HalfAway>(120);

        let fresh: MathCache<10> = MathCache::new();
        let direct = fresh.ln2::<mode::HalfAway>(120);
    ...
    fn test_ln_base() {
        ...
        let cache: MathCache<2> = MathCache::new();
        let ln_base = cache.ln_base::<mode::HalfAway>(50);
        let ln2 = cache.ln2::<mode::HalfAway>(50);
        assert_eq!(ln_base, ln2);

        // power-of-two base: ln(8) = 3·ln(2)
        let cache8: MathCache<8> = MathCache::new();
        let ln8 = cache8.ln_base::<mode::HalfAway>(50);
        let expected = FBig::from(3) * cache8.ln2::<mode::HalfAway>(50);
    ...
    fn test_debug_does_not_dump_bigints() {
        let cache: MathCache<10> = MathCache::new();
        let _ = cache.pi::<mode::HalfAway>(100);
        let s = format!("{:?}", cache);
