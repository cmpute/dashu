...
use crate::utils::{shl_radix, shr_rem_radix_in_place};
...
impl<const X: usize, const R: u8> FloatRepr<X, R> {
    fn add(self, rhs: Self) -> Approximation<Self, Rounding> {
        ...
        if ediff > precision {
            let adjust = Rounding::from_fract::<X, R>(&lhs.mantissa, rhs.mantissa, ediff);
            lhs.mantissa += adjust;
        ...
        if fract.is_zero() {
            ...
            let adjust = Rounding::from_fract::<X, R>(&mantissa, fract, fract_prec);
