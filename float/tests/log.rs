...
fn test_ln_binary() {
    ...
    let inexact_cases = [
        ...
        (fbig!(0x3).with_precision(200).value(), fbig!(0x8c9f53d5681854bb520cc6aa829dbe5adf0a216cdbf046f81ep-199)),
...
fn test_ln_decimal() {
    ...
    let inexact_cases = [
        ...
        (dbig!(3).with_precision(60).value(), dbig!(109861228866810969139524523692252570464749055782274945173469e-59), NoOp),
...
fn test_ln_1p_binary() {
    ...
    let inexact_cases = [
        ...
        (fbig!(1).with_precision(200).value(), fbig!(0xb17217f7d1cf79abc9e3b39803f2f6af40f343267298b62d8ap-200)),
        ...
        (fbig!(0x123456789012345678901234567890123456789p-156), fbig!(0x8cb0c45979137478e2c15022d0ec8a3bc2bd96fp-159)),
        (fbig!(-0x12p-8), fbig!(-0x95p-11)),
        (fbig!(-0x1234p-16), fbig!(-0x970fp-19)),
        (fbig!(-0x123456789p-36), fbig!(-0x9712b5164p-39)),
        (fbig!(-0x123456789012345678901234567890123456789p-156), fbig!(-0x9712b51649e8249cb09abdb00a6a0fa1977d537p-159)),
    ];
    for (x, ln) in &inexact_cases {
        assert_eq!(x.ln_1p(), *ln);
        if let Inexact(v, e) = x.context().ln_1p(x) {
            assert_eq!(v, *ln);
            assert_eq!(e, NoOp);
        } else {
            panic!("the result should be inexact!")
...
fn test_ln_1p_decimal() {
    ...
    let inexact_cases = [
        ...
        (dbig!(1).with_precision(60).value(), dbig!(693147180559945309417232121458176568075500134360255254120680e-60), NoOp),
        ...
        (dbig!(987654321098765432109876543210987654321e-40), dbig!(941872150698134322883564967798628924618e-40), NoOp),
        (dbig!(-98e-3), dbig!(-10e-2), NoOp),
        (dbig!(-9876e-5), dbig!(-1040e-4), SubOne),
        (dbig!(-987654321e-10), dbig!(-103989714e-9), SubOne),
        (dbig!(-987654321098765432109876543210987654321e-40), dbig!(-103989713536376403613468637446294968790e-39), SubOne),
    ];
    for (x, ln, rnd) in &inexact_cases {
        assert_eq!(x.ln_1p(), *ln);
        if let Inexact(v, e) = x.context().ln_1p(x) {
